...
let ProductIncreaseModel = require("../model/product-send-request.model.js");
...
//Request to admin to increase product quantity (Send requests)
let productQuantityIncreaseRequest = (req, res) => {

    let productQuantityIncrease = new ProductIncreaseModel({
        name: req.body.name,
        requestToIncreaseQuantity: req.body.requestToIncreaseQuantity
    });

    productQuantityIncrease.save((err, result) => {
        // console.log(result);
        if(result) {
            res.json(result);
        } else {
...
module.exports = { login, signup, getAllAdminDetails, addEmployeeDetails, deleteEmployeeByEmail,
				...
				updateRequests, lockUserAccount, updateProductQuantity, productQuantityIncreaseRequest, UnlockUserAccount};
