...
export class UserPanelComponent implements OnInit {
  ...
  constructor(public router:Router, private userService:UserService, private route:ActivatedRoute, private orderService:OrderService) { }
  ...
  orderStatus(): void {
    ...
    this.userService.getUserFromId(this.userId).subscribe(data=>{
      ...
      this.orderService.retrieveFromEmail(userEmail).subscribe(data=> {
        for (let i in data){
          orderItemsString="";
          orderItemList =i.orderItems;
          for (let j in orderItemList){
            orderItemsString+=`${j.Quantity} ${j.product.name},`;
          }
          orderStatus = i.status;
          order = {item:orderItemsString,status:orderStatus}
          orders.push(order);
        }
